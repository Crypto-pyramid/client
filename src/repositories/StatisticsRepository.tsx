...
export class UsersByDay {
  ...
  constructor(readonly date: string, readonly count: number) {}
}
...
export class StatisticsRepository {
  ...
  async getFollowers(page: number, size: number = 10,): Promise<Pagination<Follower>> {
    const { data } = await this.a.get<PaginationJSON<FollowerSchema>>(
      `/followers`, { params: { page, size } });
